import { MapPin, Filter, AlertTriangle, Droplets, Thermometer, Activity, Layers } from 'lucide-react';
      ...
      <div className="flex-1 bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col md:flex-row relative">
        ...
        <div className="w-full md:w-80 bg-white border-l border-slate-200 flex flex-col shrink-0 z-10">
          <div className="p-4 border-b border-slate-200 bg-slate-50">
            <h3 className="font-bold text-slate-900">{t('map.advisories')}</h3>
            <p className="text-xs text-slate-500">{t('map.advisories.subtitle')}</p>
          </div>
          ...
          <div className="p-4 border-t border-slate-200 bg-slate-50">
            <button className="w-full bg-sky-600 text-white py-2.5 rounded-lg text-sm font-medium hover:bg-sky-700 transition-colors flex items-center justify-center gap-2">
              <AlertTriangle className="w-4 h-4" />
              Report Issue in Area
            </button>
          </div>
        </div>
      </div>
